...
const card=item=>`<figure data-item-id="${escapeHtml(item.id)}"><button class="work-button" data-full="${escapeHtml(item.full)}" data-title="${escapeHtml(item.title)} — ${escapeHtml(item.date)}"><img src="${escapeHtml(item.thumb)}" alt="${escapeHtml(item.alt)}" loading="lazy"></button><figcaption><span>${escapeHtml(item.title)}</span><span>${escapeHtml(item.date)}</span></figcaption><div class="item-tags">${item.tags.map(tag=>`<span class="item-tag">${escapeHtml(tag)}</span>`).join('')}</div></figure>`;
