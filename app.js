...
if(dialog){
  ...
  document.addEventListener('click',event=>{const button=event.target.closest('[data-full]');if(!button)return;image.src=button.dataset.full;image.alt=button.querySelector('img').alt;title.textContent=button.dataset.title||'';title.hidden=!title.textContent;dialog.showModal()});
